...
/* setup server parameters */
	...
	int i, j, k, bytes,bytesTEMP,displayFLAG=0;
...
/* convert the received data to OpenCV's IplImage format, thread safe */
	...
	for (j = 0; j < img->width; j++) 
	{
		...
		((uchar*)(red->imageData + i * red->widthStep))[j] = sockdata[3*k];
		((uchar*)(green->imageData + i * green->widthStep))[j] = sockdata[3*k+1];
		((uchar*)(blue->imageData + i * blue->widthStep))[j] = sockdata[3*k+2];
	...
	cvMerge(red,green,blue,NULL,img);
